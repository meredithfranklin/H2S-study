...
carson_participant_data <- tibble(X = c(-118.2, -118.1),
                       Y = c(33.8, 33.7),
                       StudyID = c(1084, 1038),
...
data_exposure_static_data <- data_exposure %>%
  select(sf_loc, ll_loc) %>%
  distinct()

data_exposure_static_data <- data_exposure_static_data %>%
  mutate(dist_wrp = get_dist_wrp(sf_loc)$dist_wrp,
         MinDist = get_dist_ref(sf_loc),
         elevation = get_elevation(ll_loc),
         EVI = get_evi(sf_loc),
...
data_exposure <- data_exposure %>%
  left_join(data_exposure_static_data)
