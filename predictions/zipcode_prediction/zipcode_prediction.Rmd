...
zipcode_data$day <- force_tz(zipcode_data$day, tz = 'America/Los_Angeles')
...
zipcode_result <- zipcode_data %>%
  ungroup() %>%
  mutate(daily_h2s_log_prediction = predict(fit.xgb_da_log_h2s_dis_ind, newdata = zipcode_data_mat))

zipcode_result$daily_h2s_prediction <- exp(zipcode_result$daily_h2s_log_prediction)

write_csv(zipcode_result, 'zipcode_grid_result.csv')
```
