...
print(paste0('Number of cells at 500m resolution: ', sum(grd_ind_500$cells)))
```

```{r}
temp <- st_bbox(la_county_full[13,]) %>%
    st_as_stars(dx = 500)

temp2 <- st_bbox(la_county_full[16,]) %>%
    st_as_stars(dx = 500)

grd_df <- rbind(as.data.frame(temp2) %>% filter(values == 0),
                as.data.frame(temp) %>% filter(values == 0))

raster_temp_xgb <- rasterFromXYZ(grd_df %>% select(x, y) %>% mutate(z = 1:nrow(grd_df)))
crs(raster_temp_xgb) <- CRS_UTM

leaflet() %>% addTiles() %>% 
                  addRasterImage(raster_temp_xgb,  
                                 opacity = .8)
image(temp)
```
...
```{r}
for (i in 1:length(grd_stars_500)) {
  print(image(grd_stars_500[[i]], text_values  = T))
}
```
We notice that zip code 90261 is very small and will not fit a 500m square, so we will use itself as the box. 
