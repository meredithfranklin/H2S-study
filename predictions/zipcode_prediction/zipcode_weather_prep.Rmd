...
zipcode_grid_fetched_home <- readRDS('zipcode_grid_fetched_home.rds')
...
zipcode_daily_unfetched_result <- zipcode_grid_df_cosmos %>%
  anti_join(zipcode_daily_result, join_by(lon == longitude, lat == latitude))
...
# loop through rows
for (i in 1:nrow(zipcode_grid_df_cosmos)){

  get_url <- sprintf("https://customer-archive-api.open-meteo.com/v1/archive?latitude=%s&longitude=%s&start_date=2018-10-01&end_date=2022-12-31&hourly=temperature_2m,relative_humidity_2m,precipitation,wind_speed_10m,wind_direction_10m&temperature_unit=fahrenheit&wind_speed_unit=mph&precipitation_unit=inch&timezone=America%%2FLos_Angeles&apikey=S4sRENjfQvm2aH86",
                     zipcode_grid_df_cosmos$lat[i], zipcode_grid_df_cosmos$lon[i])
  res = GET(get_url)
  data = fromJSON(rawToChar(res$content))
  if (!(is.null(try(data$error)))){
    ith_daily_exposure_result <- tibble(latitude = zipcode_grid_df_cosmos$lat[i],
                                        longitude = zipcode_grid_df_cosmos$lon[i],
                                        day = NA,
                                        daily_temp = NA,
                                        daily_hum = NA,
                                        daily_precip = NA,
    ...
    hourly_result <- tibble(latitude = zipcode_grid_df_cosmos$lat[i],
                                        longitude = zipcode_grid_df_cosmos$lon[i],
...
saveRDS(zipcode_daily_result, 'zipcode_daily_result_temp.rds')
result <- rbind(result_copy,
                zipcode_daily_result)

saveRDS(result, 'zipcode_grid_fetched_weather.rds')
```
