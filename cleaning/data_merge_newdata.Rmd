...
# monitor_hourly_weather <- readRDS('../data/weather/monitor_daily_weather.rds')
```

```{r}
# Merge with full_data using day and Monitor as keys
full_data <- full_data %>%
  left_join(monitor_daily_weather %>% mutate(day = as.POSIXct(day, format="%Y-%m-%d", tz = 'America/Los_Angeles')), join_by(Monitor, day))
```
...
```{r}
saveRDS(full_data, '../data/full_data_20240905.rds')
saveRDS(daily_full, '../data/daily_full_20240905.rds')
```
