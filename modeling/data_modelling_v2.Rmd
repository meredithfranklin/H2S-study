...
for (model in model_table_names) {
  ...
  best_model_features <- setdiff(names(best_model), 'GCV')
...
get_gam_model <- function(response, predictors, data) {
...
```{r}
# Since feb 2022
h2s_da_model_sincefeb2022 <- get_gam_model('H2S_daily_avg',
                                           final_model_features_table %>%
                                             filter(stat == 'da' & date == 'sincefeb2022') %>%
                                             pull(features) %>%
                                             unlist(),
                                           data_sincefeb2022)
