...
table1 <- base_monitor_stat %>%
  ...
  left_join(normal_stat %>% 
            ...
            join_by(Monitor)) %>%
  relocate(Monitor, `Closest Refinery`, `Normal Avg Daily Average`, `Disaster Avg Daily Average`)
...
predictors <- c('H2S_daily_avg', 'month', 'year', 'weekday', 'wd_avg', 'ws_avg', 
                ...
                'dist_dc', 'capacity', 'avg_temp', 'avg_hum', 'precip', 'inactive_2km') 
...
test <- anti_join(daily_full[complete.cases(daily_full),] %>% filter(day > '2022-01-31'), 
                  train)
