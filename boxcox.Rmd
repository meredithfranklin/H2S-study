...
```{r}
logh2s_da_sincefeb2022 <- gam(I(log(H2S_daily_avg))~s(as.numeric(month),bs='cc') + year + as.character(weekday) +
                         wd_avg + ws_avg + daily_downwind_ref + capacity +
                          I(1/dist_wrp^2) + I(1/MinDist^2) +
                         s(I(mon_utm_x/10^3), I(mon_utm_y/10^3), bs='tp', k = 10) + 
                        te(I(mon_utm_x/10^3), I(mon_utm_y/10^3), as.numeric(day),
...
```{r}
logh2s_da_dis <- gam(I(log(H2S_daily_avg))~month + as.character(weekday) +
...
```{r}
logh2s_da_excl_dis <- gam(I(log(H2S_daily_avg))~s(as.numeric(month),bs='cc') + year + as.character(weekday) +
                         wd_avg + ws_avg + daily_downwind_ref + I(1/dist_wrp^2) + I(1/MinDist^2) +
                         s(I(mon_utm_x/10^3), I(mon_utm_y/10^3), bs='tp', k = 10) + 
                         te(I(mon_utm_x/10^3), I(mon_utm_y/10^3), as.numeric(day),
                            k = c(10,10), d = c(2,1), bs = c('tp','cc')) +
...
```{r}
logh2s_da_full_dis_ind <- gam(I(log(H2S_daily_avg))~s(as.numeric(month),bs='cc') + year + as.character(weekday) +
                         wd_avg + ws_avg + daily_downwind_ref + I(1/dist_wrp^2) + I(1/MinDist^2) +
                         s(I(mon_utm_x/10^3), I(mon_utm_y/10^3), bs='tp', k = 10) + 
                         te(I(mon_utm_x/10^3), I(mon_utm_y/10^3), as.numeric(day),
                            k = c(10,10), d = c(2,1), bs = c('tp','cc')) +
...
```{r}
logh2s_da_full <- gam(I(log(H2S_daily_avg))~s(as.numeric(month),bs='cc') + year + as.character(weekday) +
                         wd_avg + ws_avg + daily_downwind_ref + I(1/dist_wrp^2) + I(1/MinDist^2) +
                         s(I(mon_utm_x/10^3), I(mon_utm_y/10^3), bs='tp', k = 10) + 
                         te(I(mon_utm_x/10^3), I(mon_utm_y/10^3), as.numeric(day),
                            k = c(10,10), d = c(2,1), bs = c('tp','cc')) +
...
for (fold in seq(1, 10)) {
  ...
  h2s_da_model_10cv <- gam(I(log(H2S_daily_avg))~s(as.numeric(month),bs='cc') + year + as.character(weekday) +
                         wd_avg + ws_avg + daily_downwind_ref + capacity +
                          I(1/dist_wrp^2) + I(1/MinDist^2) +
                         s(I(mon_utm_x/10^3), I(mon_utm_y/10^3), bs='tp', k = 10) + 
                        te(I(mon_utm_x/10^3), I(mon_utm_y/10^3), as.numeric(day),
...
for (fold in seq(1, 10)) {
  ...
  h2s_da_model_10cv <- gam(I(log(H2S_daily_avg)) ~ month + as.character(weekday) +
                         wd_avg + ws_avg + daily_downwind_ref + I(1/dist_wrp^2) + I(1/MinDist^2) +
                         s(I(mon_utm_x/10^3), I(mon_utm_y/10^3), bs='tp', k = 10) + 
                         te(I(mon_utm_x/10^3), I(mon_utm_y/10^3), as.numeric(day),
                            k = c(10,10), d = c(2,1), bs = c('tp','cc')) +
...
result_10cv <- rbind(result_10cv, tibble(Model = 'Exclude Disaster Log Transform',
                               ...
                               'Test RMSE' = RMSE(pred_10cv_excl_dis, obs_10cv_excl_dis)))
...
knitr::kable(tibble(Model = c('Since Feb 2022',
                    ...
                    'Train R-sq' = c(round(summary(h2s_da_model_f)$r.sq, 2),
                                     round(summary(logh2s_da_sincefeb2022)$r.sq, 2),
                                     round(summary(logh2s_da_dis)$r.sq, 2),
                                     round(summary(logh2s_da_excl_dis)$r.sq, 2),
                                     round(summary(logh2s_da_full_dis_ind)$r.sq, 2),
                                     round(summary(logh2s_da_full)$r.sq, 2))) %>% 
               left_join(rbind(result_10cv, tibble(Model = 'Since Feb 2022', 
                                                   '10CV AVG Train R-Sq' = 0.664,
                                                   '10CV AVG Test R-Sq' = 0.652,
                                                   'Test RMSE' = 0.357)), join_by(Model)) %>%
               left_join(rbind(xgb_result, tibble(Model = 'Since Feb 2022', 
                                                   'XGB R-Sq' = 0.777,
                                                   'XGB BT R-Sq' = NA,
                                                   'XGB RMSE' = 0.283,
                                                   'XGB BT RMSE' = NA)), join_by(Model)),
             digits = 3)
```
